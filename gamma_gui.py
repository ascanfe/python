...
def get_connected_monitors():
    """Restituisce lista di monitor collegati tramite xrandr"""
    result = subprocess.run(['xrandr', '--query'], capture_output=True, text=True)
    monitors = []
    for line in result.stdout.splitlines():
        if " connected" in line:
            # Il nome del monitor è prima di " connected"
            monitor_name = line.split()[0]
            monitors.append(monitor_name)
    return monitors

# Inizializza MONITOR_NAME come variabile globale da modificare in GUI
MONITOR_NAME = None
...
def load_icon(path, size):
    try:
        ...
        print(f"Errore caricamento immagine {path}: {e}")
        return None
...
def set_gamma_rgb(r, g, b):
    ...
    if MONITOR_NAME:
        ...
        print("Seleziona un monitor prima di applicare gamma RGB")
...
def set_gamma_uniform(val):
    ...
    if MONITOR_NAME:
        ...
        print("Seleziona un monitor prima di applicare gamma uniforme")
...
def on_monitor_selected(event):
    ...
    print(f"Monitor selezionato: {MONITOR_NAME}")
    reset_all()
...
# Carica icone statiche con percorsi assoluti
lampadina_icon = load_icon(os.path.join(BASE_DIR, "lampadina.png"), size=(64, 64))
monitor_icon = load_icon(os.path.join(BASE_DIR, "monitor.png"), size=(64, 64))

# Carica GIF animata k7.gif
k7_gif_path = os.path.join(BASE_DIR, "k7.gif")
try:
    k7_gif = Image.open(k7_gif_path)
except Exception as e:
    print(f"Errore caricamento GIF animata {k7_gif_path}: {e}")
    k7_gif = None
...
# MONITOR SELECTOR - Combobox con monitor rilevati
monitor_var = tk.StringVar()
monitors = get_connected_monitors()
if monitors:
    MONITOR_NAME = monitors[0]  # default primo monitor trovato
monitor_selector_label = ttk.Label(frame, text="Seleziona Monitor:")
monitor_selector_label.grid(row=0, column=0, sticky="w", pady=10)

monitor_selector = ttk.Combobox(frame, textvariable=monitor_var, values=monitors, state="readonly")
monitor_selector.grid(row=0, column=1, columnspan=2, sticky="ew", pady=10)
monitor_selector.bind("<<ComboboxSelected>>", on_monitor_selected)

if monitors:
    monitor_var.set(monitors[0])  # default primo monitor selezionato

# Monitor icon (spostato sotto selezione monitor)
icon_label_monitor = ttk.Label(frame, image=monitor_icon)
icon_label_monitor.grid(row=1, column=0, rowspan=3, sticky="n", padx=(0, 15), pady=10)

label_red = ttk.Label(frame, text="Red: 1.0")
label_red.grid(row=1, column=1, sticky="w", pady=10)
...
label_green.grid(row=2, column=1, sticky="w", pady=10)
...
label_blue.grid(row=3, column=1, sticky="w", pady=10)
tk.Scale(frame, from_=0.1, to=3.0, variable=blue_var, orient=tk.HORIZONTAL,
         command=update_rgb, length=400, sliderlength=30, width=30, resolution=0.1).grid(row=3, column=2, sticky="ew", pady=10)

# Lampadina icon
icon_label_lampadina = ttk.Label(frame, image=lampadina_icon)
icon_label_lampadina.grid(row=4, column=0, sticky="n", padx=(0, 15), pady=10)

label_uniform = ttk.Label(frame, text="Uniform Gamma: 1.0")
label_uniform.grid(row=4, column=1, sticky="w", pady=10)
tk.Scale(frame, from_=0.1, to=3.0, variable=uniform_var, orient=tk.HORIZONTAL,
         command=update_uniform, length=400, sliderlength=30, width=30, resolution=0.1).grid(row=4, column=2, sticky="ew", pady=10)

# Reset button
ttk.Button(frame, text="Reset", command=reset_all).grid(row=5, column=0, columnspan=3, pady=20)

# K7 GIF animata in basso a destra
k7_label = ttk.Label(frame)
k7_label.grid(row=6, column=2, sticky="se", padx=(0, 10), pady=(10, 0))
